...
function extractJSDocDescription(content: string, interfaceName: string): string {
  ...
  const jsdoc = match[0].match(JSDOC_REGEX);
  if (!jsdoc) {
    return '';
  }
  ...
  return jsdoc[1]
    .split('\n')
    .map((line) => line.trim().replace(JSDOC_REGEX, ''))
...
function getModelInfo(): ModelInfo[] {
  ...
  // Config Models
  const configDir = join(__dirname, '../types/config');
  ...
  // Graph Models
  const graphDir = join(__dirname, '../types/graph');
...
function updateReadme() {
  ...
  const sections = readme.split(README_REGEX);
